...
@section('content')
    <div class="px-2 flex justify-between items-center">
        ...
        <div class="flex items-center gap-2">
            <button
                class="flex items-center border border-[#d1d1d1] rounded-lg px-3 gap-3 py-2 w-fit cursor-pointer hover:bg-gray-200 transitionn font-semibold">
            ...
            <button
                class="flex items-center border bg-black text-white rounded-lg px-3 gap-3 py-2 w-fit cursor-pointer hover:bg-[#1d1d1d] transitionn font-semibold">
                <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none"
                    stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                    class="icon icon-tabler icons-tabler-outline icon-tabler-download">
                    <path stroke="none" d="M0 0h24v24H0z" fill="none" />
    ...
    <div x-data="{ tab: 'overview' }" x-cloak>
        <div class="flex bg-[#eaeaea] p-1 rounded-2xl mt-8 mx-2">
            <button @click="tab = 'overview'" :class="tab === 'overview' ? 'bg-white' : ''"
                class="flex-1 text-center py-2 rounded-full text-sm font-semibold hover:bg-white transition">
                Overview
            </button>

            <button @click="tab = 'batch'" :class="tab === 'batch' ? 'bg-white' : ''"
                class="flex-1 text-center py-2 rounded-full text-sm font-semibold hover:bg-white transition">
                Batch
            </button>

            <button @click="tab = 'peserta'" :class="tab === 'peserta' ? 'bg-white' : ''"
                class="flex-1 text-center py-2 rounded-full text-sm font-semibold hover:bg-white transition">
                Peserta
            </button>

            <button @click="tab = 'performa'" :class="tab === 'performa' ? 'bg-white' : ''"
                class="flex-1 text-center py-2 rounded-full text-sm font-semibold hover:bg-white transition">
                Performa
            </button>
        </div>
                            ...
                            <script>
                                new Chart(document.getElementById('statusBatch'), {
                                    ...
                                    data: {
                                        ...
                                        datasets: [{
                                            data: [{{ $batchCounts['scheduled'] ?? 0 }},
                                                {{ $batchCounts['ongoing'] ?? 0 }},
                            ...
                            <div
                                ...
                                <div>
                                    ...
                                    <p class="text-md font-medium text-[#737373] flex flex-wrap gap-2">
                                        <span>{{ $batch->display_code ?? '-' }}</span>
                                        <span>•</span>
                                        <span>{{ $batch->category->name }}</span>
                                    </p>
                                    <p class="text-md font-medium text-[#737373] flex flex-wrap gap-2">
                                        <span>{{ \Carbon\Carbon::parse($batch->start_date)->format('d/m/Y') }} </span>
                                        <span>-</span>
                                        <span>{{ \Carbon\Carbon::parse($batch->end_date)->format('d/m/Y') }}</span>
                                    </p>
                                </div>
                                ...
                                <div>
                                    <span
                                        class="px-3 py-1 text-sm uppercase font-medium rounded-full {{ $statusBatch }}"">
                                        {{ $batch->status }}
                                    </span>
                                    <p class="text-md font-medium text-[#737373] pt-2 text-right">
                                        {{ $batch->participants_count ?? 0 }} peserta
                                    </p>
                                </div>
                            </div>
                        @endforeach
        ...
        <div x-show="tab === 'performa'">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-4 mt-8 px-2">
                ...
                <div class="bg-white border rounded-2xl p-6 mx-2">
                    ...
                    <script>
                        const kategoriLabels = @json($categories->pluck('name'));
                        const batchCounts = @json($batchPerKategori);
                ...
                <div class="bg-white border rounded-2xl p-6 max-h-[380px] overflow-y-auto">
                    ...
                    <div class="mb-4">
                        <div class="p-4 rounded-xl font-medium text-[#10AF13] bg-green-100 border border-green-300">
                            ...
                            <h2 class="pt-1">
                                {{ $kategoriPelatihanCounts['totalKategori'] ?? 0 }} Kategori
                            </h2>
                        </div>
                    </div>
                </div>
