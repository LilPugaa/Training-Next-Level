...
@section('content')
    ...
    <div x-show="openAddUser" x-cloak x-transition class="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
        <div class="bg-white w-full max-w-xl rounded-2xl p-6 relative max-h-[89vh] overflow-y-auto">
                ...
                <div class="space-y-4 bg-gray-50 rounded-xl px-6 py-1 mx-2 mb-2 pb-7">
                        ...
                        <select id="role" name="role_id" required onchange="toggleTokenField()"
                            ...
                            @foreach ($roles as $role)
                                <option value="{{ $role->id }}"
                                    data-require-token="{{ $role->name !== 'participant' ? '1' : '0' }}">
                                    {{ $role->description }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mt-4 hidden" id="tokenCreateUser">
                        ...
                        <x-text-input id="tokenInputUser"
                            class="block mt-1 w-full focus:ring-[#10AF13] focus:border-[#10AF13]" type="password"
                            name="tokenInputUser" :value="old('token')" placeholder="Masukkan token akses" />
    ...
    <div class="grid gap-6 mt-8 px-2">
        <div class="bg-white border rounded-2xl p-6">
                    ...
                    <tbody class="divide-y text-sm">
                        @foreach ($users as $user)
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-4 py-3">
                                    <span
                                        class="px-3 py-1 text-xs font-medium rounded-full {{ badgeRole($user->role?->name) }}">
            ...
            <!-- Modal Edit User -->
            <div x-show="openEditUser" x-cloak x-transition
                class="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
                <div class="bg-white w-full max-w-xl rounded-2xl p-6 relative max-h-[89vh] overflow-y-auto">
                    <button @click="openEditUser = false" class="absolute top-6 right-6 text-[#737373] hover:text-black">
                    ...
                    <form method="POST" :action="`/users/${selectedUser.id}`">
                        ...
                        <div class="flex justify-end gap-3 pt-4 me-2">
                            <button type="button" @click="openEditUser = false"
                                class="px-4 py-2 border rounded-lg hover:bg-gray-50 font-medium">
                                Batal
                            </button>
                            <button type="submit"
            ...
            <div x-show="openDeleteUser" x-cloak x-transition
                ...
                <div @click.outside="openDeleteUser = false" class="bg-white w-full max-w-md rounded-2xl p-6 space-y-4">
                    ...
                    <div class="flex justify-end gap-3 pt-4">
                        <button @click="openDeleteUser = false" class="px-4 py-2 border rounded-lg hover:bg-gray-50 font-medium">
                            Batal
                        </button>
